...
#if defined(NDEBUG)
# error "SUHARCOIN cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x85ca4fcae33ef1ec9e8118131620820187149075440fff586ac402c1e41e50c5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SUHARCOIN: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SUHARCOIN Signed Message:\n";
...
{
    // SUHARCOIN: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 25000); // SUHARCOIN: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // SUHARCOIN: 1 days
static const int64 nTargetSpacing = 5 * 60; // SUHARCOIN: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // SUHARCOIN: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("suharcoin-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // SUHARCOIN: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x85ca4fcae33ef1ec9e8118131620820187149075440fff586ac402c1e41e50c5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "LakoMoor Dev First block SUHARa";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04eb2fb57e1199a21aee6019a6bbb63ecec29f18aca13e111d73d0db8993021860b7c4da0d9f693d34671b5252dd342c8b18403398e9ce3017bf5956bbe5cf4bda") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1642257400;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086099650;
        ...
        assert(block.hashMerkleRoot == uint256("0x826e3fc7ce1c8d457725f4b48b039c3540e5ccb02353acf6244093da9cbb4b8b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xb6, 0xb7, 0x87, 0xbb }; // SUHARCOIN: increase each by adding 2 to suharcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SUHARCOINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SUHARCOINMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SUHARCOINMiner(CWallet *pwallet)
{
    printf("SUHARCOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("suharcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SUHARCOINMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SUHARCOINMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SUHARCOINMiner, pwallet));
}
